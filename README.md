# Dartmouth Code Writers Guild: Reading Group

![Meetings](https://img.shields.io/badge/meetings-monthly-blue)
![Duration](https://img.shields.io/badge/duration-45min-green)
![Format](https://img.shields.io/badge/format-hybrid-purple)

A monthly forum for Dartmouth members who write code to discuss software engineering practices, tools, and ideas through bite-sized content.

## 🎯 Purpose

This reading group brings together software engineers, research software engineers, and anyone who writes code at Dartmouth to:

- **Learn** about industry practices and emerging ideas in software engineering
- **Discuss** how these concepts apply to our work at Dartmouth
- **Share** experiences and perspectives from our diverse projects
- **Improve** our craft through collective learning

We focus on practical, actionable content that can influence how we build software.

## 📅 Format

- **Frequency:** Monthly (typically 2nd Thursday, 2:00-2:45 PM ET)
- **Duration:** 45 minutes
- **Location:** Hybrid (room TBD + video conference link in calendar invite)
- **Materials:** Usually short-form content (~15-30 min reading/watching), occasionally longer
  - Blog posts
  - Conference talks
  - Style guides
  - Documentation
  - Short articles or papers

### Typical Session Flow

```
0:00-0:05  Welcome & recap
0:05-0:10  Brief summary of material (facilitator)
0:10-0:40  Facilitated discussion
0:40-0:45  Takeaways & next topic selection
```

## 🚀 Getting Started

### For Participants

1. **Join the group:** Watch this repository to get notifications
2. **Review materials:** Check the [upcoming session](#-upcoming-sessions) 1-2 weeks before
3. **Engage:** Add thoughts to the discussion thread before the meeting
4. **Attend:** Join us live or catch up via meeting notes afterward
5. **Contribute:** Suggest topics via [GitHub Issues](../../issues/new?template=topic-suggestion.md)

### For First-Timers

No preparation required! You can:
- Come to any session (no need to attend all)
- Participate as much or as little as you want
- Skip the reading and just listen
- Join remotely or in person

We're here to learn together, not quiz each other.

## 📚 Upcoming Sessions

### Next Session: [Date TBD]
**Topic:** TBD
**Material:** Link TBD
**Facilitator:** TBD
**Discussion:** [Link to GitHub Discussion thread]

> Check back soon for our first session announcement!

## 📖 Past Sessions

Session archives available in [`/sessions`](./sessions/) directory.

<!--
Example entry:
- **[March 2024: Code Review Best Practices](./sessions/2024-03/)** - Discussion of Google's Engineering Practices guide
-->

## 💡 Suggest a Topic

Have a blog post, video, or article that sparked ideas? We want to hear about it!

**Good topics are:**
- ✅ Short (< 30 min to consume)
- ✅ Relevant to software development practices
- ✅ Discussion-worthy (not just tutorials)
- ✅ Accessible to a broad audience

**Examples:**
- Testing strategies
- Code review practices
- Documentation approaches
- Development workflows
- Software design principles
- Team collaboration practices
- Tool comparisons
- Technical decision-making

[**→ Submit a topic suggestion**](../../issues/new?template=topic-suggestion.md)

## 👥 Facilitators

Facilitation rotates among volunteers. **No expertise required** – just:
- Select or help select the material
- Post discussion questions 2 weeks ahead
- Guide conversation during the session
- Help capture notes

Interested in facilitating? Add your name here via PR or contact [Simon](mailto:simon.stone@dartmouth.edu?subject=Code%20Writers%20Guild).

**Current facilitators:**
- [Simon Stone](mailto:simon.stone@dartmouth.edu?subject=Code%20Writers%20Guild)
- [Beaudry Chase](mailto:beaudry.chase@dartmouth.edu?subject=Code%20Writers%20Guild)

## 🤝 Discussion Guidelines

- **Be curious:** Ask questions and assume good intent
- **Be inclusive:** Make space for all voices
- **Be respectful:** Disagree with ideas, not people
- **Be practical:** Connect concepts to real work
- **Be open:** "Yes, and..." over "No, but..."

See our full [Code of Conduct](./CODE_OF_CONDUCT.md).

## 📁 Repository Structure

```
.
├── README.md                          # This file
├── CODE_OF_CONDUCT.md                 # Community norms
├── .github/
│   ├── ISSUE_TEMPLATE/
│   │   ├── topic-suggestion.md        # Template for suggesting topics
│   │   ├── feedback.md                # Template for general feedback
│   │   └── session-retrospective.md   # Template for post-session feedback
│   └── DISCUSSION_TEMPLATE/
│       └── session-discussion.md      # Template for session threads
├── sessions/                          # Notes from previous sessions
│   └── template/                      # Template for new session notes
└── resources/
    ├── discussion-template-guide.md   # Template for new session discussion thread
    └── facilitation-guide.md          # How to facilitate a session
```

## 🔔 Stay Updated

- **Watch this repo:** Click "Watch" → "All Activity" at the top
- **Check Discussions:** New session announcements posted 2 weeks ahead
- **Calendar:** Accept the recurring calendar invite
- **Slack:** Follow #code-writers-guild

## 📊 Feedback & Improvements

This is an experiment! Help us improve:
- [Open an issue](../../issues/new?template=feedback.md) with suggestions
- Submit PRs to improve documentation

## ❓ Questions?

- **About the format?** Check [Discussions](../../discussions)
- **About a specific session?** Comment on that session's discussion thread
- **About facilitating?** See the [facilitation guide](./resources/facilitation-guide.md)
- **Something else?** Contact [Simon](mailto:simon.stone@dartmouth.edu?subject=Code%20Writers%20Guild) or [Beaudry](mailto:beaudry.chase@dartmouth.edu?subject=Code%20Writers%20Guild)

---

## 📜 License

Materials and notes in this repository are shared under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) unless otherwise noted. Respect original content licenses.

---

**Let's build better software together! 🚀**
